...
from open_table_connector.local_files.sdk_temporal import _csv_uri, _managed_uri
...
def test_csv_logical_and_physical_targets_are_canonical_file_uris(tmp_path: Path) -> None:
    source = tmp_path / "ticks.csv"

    physical_target = _csv_uri(source)
    logical_target = _managed_uri(source)

    assert physical_target.value == source.absolute().as_uri()
    assert physical_target.scheme == "file"
    assert logical_target.value == source.absolute().as_uri()
    assert logical_target.scheme == "file"
