...
def provider(process):
    ...
    return p, p.bind(SpreadsheetTarget("maybe://doc"))
...
def test_create_is_buffered_and_retains_workbook_id():
    ...
    assert b["uri"] == "maybe://created-doc"
...
def test_live_disposable_sheet_contract():
    ...
    b = p.bind(SpreadsheetTarget("maybe://otc-disposable-" + uuid.uuid4().hex))
    b["new"] = True
...
def test_actual_recorded_creation_url_retains_gid():
    ...
    b = p.bind(SpreadsheetTarget("maybe://fixture-document"))
    r = p.commit(
        b,
        [change("worksheet.create", "New")],
        allow_partial=False,
