...
def _destination(destination):
    if isinstance(destination, SheetModeDestination):
        uri, sheet, anchor, header = destination.grid.value, destination.worksheet, destination.anchor, destination.header
        ...
        uri, sheet, anchor, header = parsed._replace(fragment="").geturl(), selectors[0][1], "A1", True
    else:
    ...
    if not path.is_absolute() or path.suffix.lower() != ".xlsx" or not sheet or anchor != "A1" or header is not True:
...
def _metadata(source, table_id, worksheet):
    return json.dumps({
        "version": 1, "table_id": table_id, "worksheet": worksheet, "anchor": "A1", "header": True,
        "schema": _schema_to_wire(source.schema), "rows": source.height,
        "schema_fingerprint": MaterializationRequest(source, DirectDestination("file:///placeholder.xlsx"), "otc.portable-table/v1", "metadata").schema_fingerprint,
    ...
    try:
        ...
        for sheet in book.worksheets:
            ...
            try:
                ...
                continue
            if not isinstance(record, dict) or set(record) != {"version", "table_id", "worksheet", "anchor", "header", "schema", "rows", "schema_fingerprint"}:
                continue
            if record["version"] != 1 or record["anchor"] != "A1" or record["header"] is not True:
                continue
            if (table_id is None or record["table_id"] == table_id) and (worksheet is None or record["worksheet"] == worksheet):
...
def _typed_frame(path, record, schema):
    ...
    try:
        ...
        rows = [[("" if sheet.cell(row, column + 1).data_type == "inlineStr" and sheet.cell(row, column + 1).value is None else sheet.cell(row, column + 1).value) for column in range(len(schema))] for row in range(2, record["rows"] + 2)]
        ...
        for name, dtype in schema.items():
            if dtype == pl.String:
                ...
                expressions.append(pl.when(pl.col(name).is_null()).then(None).otherwise(pl.col(name) == "true").cast(dtype).alias(name))
            else:
...
def open_portable_excel(address):
    path = Path(unquote(urlsplit(address.grid.value).path)) if isinstance(address, SheetModeTableAddress) else None
    ...
    return TableBinding(path.as_uri(), "sheet-mode", frame.schema, revision, "local-files", "otc.portable-table/v1", frame.height, record["schema_fingerprint"], None, canonical), frame
...
def create_portable_excel_table(connector, request: MaterializationRequest):
    ...
    try:
        ...
        session = SpreadsheetSession(provider, SpreadsheetTarget(uri), new=not path.exists(), profile="general/1.0")
        try:
            existing = set(session.observe("worksheet.list")["value"])
            if worksheet.casefold() in {name.casefold() for name in existing}:
                return OperationResult(None, Outcome.REJECTED, CommitState.NOT_STARTED, VerificationState.SKIPPED, (), error=ErrorInfo(ErrorCode.DESTINATION_EXISTS, "Excel destination worksheet already exists"))
            metadata_sheet = _metadata_name(table_id, existing | {worksheet})
            session.queue("worksheet.create", worksheet, {"name": worksheet})
            session.queue("range.write", worksheet, {"address": f"A1:{chr(64 + request.source.width)}{request.source.height + 1}", "values": [request.source.columns, *[[_cell(value) for value in row] for row in request.source.iter_rows()]]})
            session.queue("worksheet.create", metadata_sheet, {"name": metadata_sheet})
            session.queue("range.write", metadata_sheet, {"address": "A1", "values": [[_metadata(request.source, table_id, worksheet)]]})
            session.queue("worksheet.config", metadata_sheet, {"properties": {"visibility": "veryHidden"}})
            written = session.write(expected_revision=session.binding.get("revision"))
        finally:
            session.close()
        mutation = (Receipt("physical", "table.materialize.create", connector.identity.connector_id, "table.materialize.create/1.0", uri, "sheet-mode", {"revision": written["binding"]["revision"]}),)
        ...
        if not observed.equals(request.source):
            return OperationResult(None, Outcome.FAILED, CommitState.COMMITTED, VerificationState.FAILED, (mutation, read), error=ErrorInfo(ErrorCode.READBACK_MISMATCH, "portable Excel readback differs from submitted table", {"expected": str(request.source), "observed": str(observed)}))
        return OperationResult(binding, Outcome.SUCCEEDED, CommitState.COMMITTED, VerificationState.PASSED, (*mutation, read))
    except ConnectorError as exc:
        if written is not None:
            return OperationResult(None, Outcome.FAILED, CommitState.COMMITTED, VerificationState.FAILED, mutation, error=ErrorInfo(ErrorCode.READBACK_MISMATCH, "portable Excel commit completed but readback failed", {"cause": type(exc).__name__}))
        code = ErrorCode.STALE_REVISION if exc.code.value == "conflict" and "stale" in exc.message.lower() else ErrorCode.EXECUTION_FAILED
        return OperationResult(None, Outcome.REJECTED, CommitState.NOT_STARTED, VerificationState.SKIPPED, (), error=ErrorInfo(code, exc.message, dict(exc.safe_details)))
...
def create_excel_table(connector, source, destination):
    ...
    try:
        ...
        selectors = parse_qsl(parsed.fragment, keep_blank_values=True)
        if (parsed.scheme != "file" or parsed.netloc not in ("", "localhost") or parsed.query or len(selectors) != 1 or selectors[0][0] != "sheet" or not selectors[0][1]):
            raise ValueError("Excel materialization requires file:///absolute/book.xlsx#sheet=Name")
        ...
        book = client.workbook.open(path.as_uri()) if path.exists() else client.workbook.create(path.as_uri(), profile="general/1.0")
        ...
        if written.outcome is not Outcome.SUCCEEDED or written.commit is not CommitState.COMMITTED or written.verification is not VerificationState.PASSED:
        ...
        if written is not None:
            return OperationResult(None, Outcome.FAILED, written.commit, VerificationState.FAILED, receipts, error=ErrorInfo(ErrorCode.EXECUTION_FAILED, "Excel table readback failed", {"cause": str(exc), "commit": written.commit.value}))
