...
Only canonical HTTPS document URLs are accepted, and new wire data and receipts
use that same canonical container.
...
- Existing Maybe append writes remain a separate capability and are never used
  by `materialize`.
- Existing reads and new materialization requests, returned addresses, and
  receipts use canonical HTTPS containers.
