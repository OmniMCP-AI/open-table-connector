"""Scoped credential resolution for SDK connectors."""
...
from open_table_connector.contract import ProviderConfig
...
def _credential_error(code: ErrorCode, message: str, **details: object) -> OTCError:
...
class EnvironmentCredentialResolver:
    def __init__(self, config: ClientConfig, environ: Mapping[str, str]) -> None:
        self._config = config
...
def apply_credential_overrides(config: ClientConfig, overrides: Mapping[str, str]) -> ClientConfig:
    ...
    for raw_provider_id, raw_reference in overrides.items():
        ...
        providers[provider_id] = (
            ProviderConfig(provider_id, enabled=True, credential_reference=reference)
            if provider is None
            else ProviderConfig(
    ...
    return ClientConfig(providers=providers, credentials=config.credentials)
